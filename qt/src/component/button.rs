...
impl State {
    ...
    fn get_desired_icon_spacing(&self) -> f32 {
        let tokens = &self.qt.theme.tokens;
        match &self.size {
...
fn on_mouse_enter(window: &HWND, context: &Context) -> Result<()> {
...
fn on_mouse_leave(context: &Context) -> Result<()> {
    _ = change_color(context);
    Ok(())
}

fn on_mouse_click(window: &HWND, context: &Context) -> Result<()> {
