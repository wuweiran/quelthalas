...
// Icon slots (logical DIPs). The leading search glyph and the trailing clear (X)
// button, both re-tinted per paint.
const SEARCH_ICON: f32 = 20.0;
const DISMISS_ICON: f32 = 20.0;
/// The clear button's square hover target (DIPs); wider than the glyph.
const DISMISS_BOX: f32 = 24.0;
...
impl Context {
    ...
    /// Cached text layout over the field text.
    fn update_text_layout(&mut self) -> Result<Option<IDWriteTextLayout>> {
        if self.text_layout.is_none() {
            ...
            let display: Vec<u16> = unsafe { self.buffer.as_wcs().as_wide()[..length].to_vec() };
            let layout = unsafe {
...
impl QT {
    pub fn create_search_box(
        ...
        let class_name: PCWSTR = w!("QT_SEARCH_BOX");
        unsafe {
...
fn adjust_format_rect(window: HWND, context: &mut Context) -> Result<()> {
    // NOTE: unlike `input.rs`, we do NOT expand `format_rect.right` to the text
    // extent (`left + char_width`). In `input` that expansion is dormant — its
    // `set_rect_np` only runs on create/DPI-change, when the text is empty — but the
    // search box re-runs `set_rect_np` on focus/blur with text present, which would
    // push the right edge past the reserved clear-button slot and let text render
    // under the X. Here `format_rect.right` is always the fixed field edge.
...
fn slot_rects(window: HWND, context: &Context) -> (RECT, RECT) {
    let scaling_factor = get_scaling_factor(window);
    let mut rc = RECT::default();
    unsafe {
    ...
    let border = (1.0 * scaling_factor) as i32;
    let s = (tokens.spacing_horizontal_s * scaling_factor) as i32;
    let search_w = (SEARCH_ICON * scaling_factor) as i32;
    let dismiss_w = (DISMISS_ICON * scaling_factor) as i32;
    // Leading search glyph: S in from the left border.
    let search_left = rc.left + border + s;
    ...
    // Trailing clear glyph: S in from the right border. The hover target is a
    // DISMISS_BOX-square, centered on the glyph and vertically centered.
    let dismiss_right = rc.right - border - s;
    let box_w = (DISMISS_BOX * scaling_factor) as i32;
    let box_h = box_w;
    let center_x = dismiss_right - dismiss_w / 2;
    ...
    (search_rect, dismiss_rect)
}

/// Is device-pixel point `(x, y)` inside `rect`?
fn point_in(rect: &RECT, x: i32, y: i32) -> bool {
    x >= rect.left && x < rect.right && y >= rect.top && y < rect.bottom
}

/// Clear the field and keep keyboard focus (caret returns to the start).
fn clear_search(window: HWND, context: &mut Context) -> Result<()> {
    let length = context.get_text_length();
...
/// Re-run the layout when the clear button's visibility might change (focus +
/// has-text). `set_rect_np` reserves the trailing slot in `format_rect` and sets
/// `dismiss_reserved` **together**, and paint + hit-test both key off that one flag —
/// so the X is only ever drawn/clickable when its slot is actually carved out, and
/// text can never render under it.
fn reflow_dismiss(window: HWND, context: &mut Context) -> Result<()> {
    let show = context.is_focused && context.get_text_length() > 0;
    if show != context.dismiss_reserved {
        set_rect_np(window, context)?;
        scroll_caret(window, context)?;
        update_scroll_info(window, context);
        unsafe {
            _ = InvalidateRect(Some(window), None, false);
...
fn set_rect_np(window: HWND, context: &mut Context) -> Result<()> {
    ...
    // Content padding + icon slots. Fluent SearchBox insets:
    //   S + search + SNudge + [text] + SNudge + dismiss + S   (when the X shows)
    //   S + search + SNudge + [text] + S                      (when it doesn't)
    let tokens = &context.state.qt.theme.tokens;
    let s = (tokens.spacing_horizontal_s * scaling_factor) as i32;
    let nudge = (tokens.spacing_horizontal_s_nudge * scaling_factor) as i32;
    let search_w = (SEARCH_ICON * scaling_factor) as i32;
    let dismiss_w = (DISMISS_ICON * scaling_factor) as i32;
    context.format_rect.left += s + search_w + nudge;
    // The clear button only shows while the box is focused and has text; its slot is
    // reserved only then, so the text area extends to the full `S` padding otherwise.
    let show_dismiss = context.is_focused && context.get_text_length() > 0;
    context.format_rect.right -= if show_dismiss { nudge + dismiss_w + s } else { s };
    context.dismiss_reserved = show_dismiss;
...
fn on_kill_focus(window: HWND, context: &mut Context) -> Result<()> {
    ...
    // The clear button hides on blur, so the text area reflows back to full width.
    reflow_dismiss(window, context)?;
    unsafe {
...
fn on_paint(window: HWND, context: &mut Context) -> Result<()> {
    ...
    unsafe {
        rt.BeginDraw();
        let result =
            paint_content_and_chrome(window, context).and_then(|_| paint_icons(window, context));
        let end = rt.EndDraw(None, None);
        result.and(end)
    }
}

/// Draw the leading search glyph and the trailing clear (X) button over the chrome.
/// Split out of `paint_content_and_chrome` so it can take the `&mut` borrow that
/// `get_text_length` needs without fighting the `tokens` borrow there.
fn paint_icons(window: HWND, context: &mut Context) -> Result<()> {
    let scaling_factor = get_scaling_factor(window);
    let (search_rect, dismiss_rect) = slot_rects(window, context);
    // The clear button shows only while focused with text (matches the reserved slot).
    let show_dismiss = context.is_focused && context.get_text_length() > 0;
...
fn set_focus(window: HWND, context: &mut Context) -> Result<()> {
    ...
    context.caret_visible = true;
    // The clear button shows on focus when there's text, so reflow the text area.
    reflow_dismiss(window, context)?;
    invalidate_text(
        window,
        context,
...
extern "system" fn window_proc(
    ...
    match message {
        ...
        WM_LBUTTONDOWN => unsafe {
            ...
            // Clear button (only shown while focused with text) takes precedence.
            let (_search, dismiss) = slot_rects(window, context);
            if context.is_focused
                && context.get_text_length() > 0
                && point_in(&dismiss, mouse_x, mouse_y)
            {
                context.dismiss_pressed = true;
                SetCapture(window);
                return LRESULT(0);
            }
        ...
        WM_LBUTTONUP => unsafe {
            let raw = GetWindowLongPtrW(window, GWLP_USERDATA) as *mut Context;
            let context = &mut *raw;
            if context.dismiss_pressed {
                context.dismiss_pressed = false;
                if GetCapture() == window {
                    _ = ReleaseCapture();
                }
                let mouse_x = l_param.0 as i16 as i32;
                let mouse_y = (l_param.0 >> 16) as i16 as i32;
                let (_search, dismiss) = slot_rects(window, context);
                if point_in(&dismiss, mouse_x, mouse_y) {
                    _ = clear_search(window, context);
                }
                return LRESULT(0);
            }
        ...
        WM_MOUSEMOVE => unsafe {
            ...
            let mouse_x = l_param.0 as i16 as i32;
            let mouse_y = (l_param.0 >> 16) as i16 as i32;
            // Track hover over the clear button — only for the cursor (no repaint).
            let (_search, dismiss) = slot_rects(window, context);
            context.dismiss_hovered = context.is_focused
                && context.get_text_length() > 0
                && point_in(&dismiss, mouse_x, mouse_y);
            // While clearing, don't run the text-selection drag path.
            if !context.dismiss_pressed {
        ...
        WM_SETCURSOR => unsafe {
            // Hand (finger) cursor over the clear button; else the default I-beam.
            let raw = GetWindowLongPtrW(window, GWLP_USERDATA) as *mut Context;
            if !raw.is_null() && (*raw).dismiss_hovered {
                _ = SetCursor(LoadCursorW(None, IDC_HAND).ok());
                LRESULT(TRUE.0 as isize)
            } else {
